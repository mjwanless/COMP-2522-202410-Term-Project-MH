...
public class CastleScreen implements Screen {
    ...
    private Music castleScreenMusic;
    ...
    private void setupUI() {
        ...
        TextButton bossKilledButton = new TextButton("Go to Desert", skin);
        bossKilledButton.setSize(750, 100); // Set the size of the button
        // Position the button at the bottom center of the screen
        bossKilledButton.setPosition((Gdx.graphics.getWidth() - bossKilledButton.getWidth()) / 2, 100); // Raise it 20px above the bottom

        bossKilledButton.addListener(new ClickListener() {
            @Override
            public void clicked(com.badlogic.gdx.scenes.scene2d.InputEvent event, float x, float y) {
                game.setScreen(new BossKilledScreen(game, selectedCharacters)); // Transition to DesertScreen
            }
        });

        stage.addActor(bossKilledButton);
    }
